...
QUIZ_QUESTIONS = {
    'Python': [
        ...
        {"question": "What does len() do?", "options": ["Returns length", "Converts to string", "Finds maximum", "Rounds number"], "correct": "Returns length", "explanation": "len() returns number of items"},
        {"question": "Which operator is for exponentiation?", "options": ["**", "^", "exp()", "&&"], "correct": "**", "explanation": "** is exponentiation operator"},
        ...
        {"question": "What is the correct while loop?", "options": ["while x > y:", "while (x > y)", "x > y while", "while x > y then"], "correct": "while x > y:", "explanation": "Colon required after condition"},
    ...
    'JavaScript': [
        ...
        {"question": "What is a closure?", "options": ["Function with outer scope", "Closed function", "Private variable", "Global variable"], "correct": "Function with outer scope", "explanation": "Closures remember outer variables"},
    ],
    'SQL': [
...
def analyze_resume(text):
    ...
    for role, score in sorted(scores.items(), key=lambda x: x[1], reverse=True):
        if score >= 30 and role != best_role:
            suitable_roles.append({'role': role, 'score': score, 'skills': matched_skills[role][:3]})
    
    strengths = []
    if len(text) > 500:
        strengths.append("✅ Good resume length")
    if '@' in text:
        strengths.append("✅ Contact info included")
    if best_score >= 60:
    ...
    if best_score < 50:
        improvements.append(f"📈 Add {best_role} keywords")
    
    if not strengths:
        strengths = ["✅ Resume uploaded"]
    if not improvements:
        improvements = ["📈 Add achievements with numbers"]
    ...
    return {
        ...
        'skills': list(set([kw for skills in matched_skills.values() for kw in skills]))[:8]
    }
    ...
    for q in INTERVIEW_QUESTIONS.get(role, []):
        if q['question'] == question:
            ...
            if score >= 80:
                ...
                feedback = f"👍 Good! Also mention: {', '.join([k for k in keywords if k not in matched][:2])}"
            else:
                feedback = f"📝 Fair. Missing: {', '.join([k for k in keywords if k not in matched][:2])}"
...
def resume_analysis():
    if request.method == 'POST':
        ...
        if not file.filename.endswith('.pdf'):
            flash('Please upload PDF file', 'danger')
            return redirect(url_for('resume_analysis'))
        ...
        try:
            ...
            flash(f'✅ Best match: {analysis["best_role"]}', 'success')
